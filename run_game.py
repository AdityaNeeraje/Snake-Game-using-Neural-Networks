...
def run_game_with_ML(display, clock, agent):
    max_score = 0
    test_games = 1
    score1 = 0
    steps_per_game = 2500
    ...
    for _ in range(test_games):
        ...
        for _ in range(steps_per_game):
            ...
            predictions = []
            predicted_direction = np.argmax(agent.forward(torch.tensor(np.array(
                [is_left_blocked, is_front_blocked, is_right_blocked, apple_direction_vector_normalized[0],
                 ...
                 snake_direction_vector_normalized[1]]).reshape(-1, 7), dtype=torch.float32)).detach().numpy()) - 1
